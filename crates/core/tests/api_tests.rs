...
// ===========================================================================
// api::search_expanded() tests
// ===========================================================================

/// search_expanded returns a SearchResult struct per match containing both the
/// symbol ID and its full source code.
#[test]
fn search_expanded_returns_id_and_source_for_matching_symbol(
) -> Result<(), Box<dyn std::error::Error>> {
    let dir = TempDir::new().expect("failed to create tempdir");
    write_rust_fixture(&dir, "lib.rs", RUST_FIXTURE);
    api::index(dir.path()).expect("index should succeed");

    let results = api::search_expanded(dir.path(), "greet")?;

    assert!(
        !results.is_empty(),
        "search_expanded should return at least one result for 'greet'"
    );

    let first = &results[0];
    assert!(
        first.id.contains("greet"),
        "SearchResult.id should contain the query substring; got: {}",
        first.id
    );
    assert!(
        first.source.contains("pub fn") || first.source.contains("pub struct"),
        "SearchResult.source should contain actual source code; got: {}",
        first.source
    );
    Ok(())
}

/// search_expanded source field contains real function body, not just the signature.
#[test]
fn search_expanded_source_contains_function_body() -> Result<(), Box<dyn std::error::Error>> {
    let dir = TempDir::new().expect("failed to create tempdir");
    write_rust_fixture(&dir, "lib.rs", RUST_FIXTURE);
    api::index(dir.path()).expect("index should succeed");

    let results = api::search_expanded(dir.path(), "add")?;

    let add_result = results
        .iter()
        .find(|r| r.id.ends_with("::add"))
        .expect("should find the 'add' function in search_expanded results");

    assert!(
        add_result.source.contains("a + b"),
        "SearchResult.source for 'add' must contain the function body 'a + b'; got: {}",
        add_result.source
    );
    Ok(())
}

/// search_expanded with no matches returns an empty vec (not an error).
#[test]
fn search_expanded_returns_empty_vec_when_no_matches() -> Result<(), Box<dyn std::error::Error>> {
    let dir = TempDir::new().expect("failed to create tempdir");
    write_rust_fixture(&dir, "lib.rs", RUST_FIXTURE);
    api::index(dir.path()).expect("index should succeed");

    let results = api::search_expanded(dir.path(), "xyzzy_does_not_exist")?;

    assert!(
        results.is_empty(),
        "search_expanded should return an empty vec when nothing matches"
    );
    Ok(())
}

/// The original api::search remains unchanged — returns Vec<String> symbol IDs only.
#[test]
fn search_unchanged_returns_only_symbol_ids() -> Result<(), Box<dyn std::error::Error>> {
    let dir = TempDir::new().expect("failed to create tempdir");
    write_rust_fixture(&dir, "lib.rs", RUST_FIXTURE);
    api::index(dir.path()).expect("index should succeed");

    let results: Vec<String> = api::search(dir.path(), "add")?;

    assert!(
        !results.is_empty(),
        "api::search should still return symbol IDs for 'add'"
    );
    // Every element must be a plain String (symbol ID), not a struct.
    for id in &results {
        assert!(
            !id.is_empty(),
            "api::search must return non-empty symbol ID strings"
        );
    }
    Ok(())
}
