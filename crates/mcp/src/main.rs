...
async fn main() -> anyhow::Result<()> {
    ...
    server.register_tool(
        Tool::new(
            "index",
            "Builds or updates the codebones index for a directory",
        )
    ...
    server.register_tool(
        ...
        |args| async move {
            let result = codebones_core::api::outline(std::path::Path::new("."), &args)
    ...
    server.register_tool(
        Tool::new(
            "get",
            "Retrieves the full source code for a specific symbol",
        )
        .with_arg("symbol", "string", "Symbol name to retrieve"),
        |args| async move {
            let result = codebones_core::api::get(std::path::Path::new("."), &args)
                .unwrap_or_else(|e| e.to_string());
            Ok(result)
        },
    );
    ...
    server.register_tool(
        Tool::new("search", "Searches for symbols across the repository").with_arg(
            "query",
            "string",
            "Search query",
        ),
        |args| async move {
            let results =
                codebones_core::api::search(std::path::Path::new("."), &args).unwrap_or_default();
            Ok(results.join("\n"))
        },
    );
